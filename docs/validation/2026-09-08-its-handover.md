...
- Automatic approval review initially blocked replacing the modified root CLAUDE.md. The owner then explicitly approved the switch. Its original content was verified against the preserved backup, and the original repository folder now checks out codex/its-handover. The temporary handover worktree is detached; no application deployment changed.
