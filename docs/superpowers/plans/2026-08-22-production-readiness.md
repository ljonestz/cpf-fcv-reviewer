...
**Architecture:** Flask enqueues expiring assessment payloads in private Redis; a separate worker executes the existing orchestrator; Redis-backed state supports reconnectable SSE and post-restart results/downloads. Local/smoke mode retains in-memory execution.

**Stack:** Python 3.13, Flask, Redis, RQ, Gunicorn, Render Blueprint, pytest, Playwright.
...
- Implement a Redis session store behind the existing API, namespaced and TTL-bound.
- Add failing enqueue-contract tests covering create, retry, correction, and test/local inline behaviour.
- Implement an RQ-backed queue adapter and an in-process adapter for tests/smoke.
...
- Add an RQ worker entry point with graceful termination and bounded job timeout/retry settings.
...
- Add configuration tests for Redis/queue mode, 24-hour production retention, job timeout, and deployed release detection.
- Add dependencies and a `render.yaml` defining web, worker, and private `noeviction` Key Value services.
